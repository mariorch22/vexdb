...
TEST(SQ8, EncodeDecodeRoundTrip) {
    ...
    auto params = vexdb::sq8_compute_params(vec.data(), 1, dim);

    std::vector<uint8_t> encoded(dim);
    vexdb::sq8_encode(vec.data(), encoded.data(), params);

    std::vector<float> decoded(dim);
    vexdb::sq8_decode(encoded.data(), decoded.data(), params);
...
TEST(SQ8, EncodeMapsMinToZeroMaxTo255) {
    ...
    auto params = vexdb::sq8_compute_params(vecs.data(), 2, dim);

    // Encode vec 0 (min values)
    std::vector<uint8_t> enc0(dim);
    vexdb::sq8_encode(vecs.data(), enc0.data(), params);
    EXPECT_EQ(enc0[0], 0);
    EXPECT_EQ(enc0[1], 0);

    // Encode vec 1 (max values)
    std::vector<uint8_t> enc1(dim);
    vexdb::sq8_encode(vecs.data() + dim, enc1.data(), params);
...
TEST(SQ8, ConstantDimensionHandled) {
    ...
    auto params = vexdb::sq8_compute_params(vecs.data(), 3, dim);
    ...
    vexdb::sq8_encode(vecs.data(), enc.data(), params);

    std::vector<float> dec(dim);
    vexdb::sq8_decode(enc.data(), dec.data(), params);
...
TEST(SQ8, AsymmetricDistanceCloseToFloat32) {
    ...
    auto params = vexdb::sq8_compute_params(vecs.data(), n, dim);

    // Encode all vectors
    std::vector<uint8_t> encoded(n * dim);
    for (size_t i = 0; i < n; i++) {
        vexdb::sq8_encode(vecs.data() + i * dim, encoded.data() + i * dim, params);
    }
    ...
    for (size_t i = 0; i < n; i++) {
        float exact = vexdb::l2_distance(query.data(), vecs.data() + i * dim, dim);
        float approx = vexdb::sq8_asymmetric_l2(query.data(), encoded.data() + i * dim, params);
...
class SQ8ConsistencyTest : public ::testing::TestWithParam<vexdb::Dim> {};

TEST_P(SQ8ConsistencyTest, AVX2MatchesScalar) {
    vexdb::Dim dim = GetParam();
    std::mt19937 rng(123);
    ...
    for (size_t i = 0; i < n; i++) {
        float scalar =
            vexdb::sq8_asymmetric_l2_scalar(query.data(), encoded.data() + i * dim, params);
        float avx2 = vexdb::sq8_asymmetric_l2(query.data(), encoded.data() + i * dim, params);
