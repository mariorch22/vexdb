...
static void BM_SQ8Asymmetric_128d(benchmark::State& state) {
    ...
    auto params = vexdb::sq8_compute_params(vecs.data(), n, 128);
    std::vector<uint8_t> encoded(n * 128);
    for (int i = 0; i < n; i++) {
        vexdb::sq8_encode(&vecs[i * 128], &encoded[i * 128], params);
    }
    ...
    for (auto _ : state) {
        for (int i = 0; i < n; i++) {
            benchmark::DoNotOptimize(
                vexdb::sq8_asymmetric_l2(query.data(), &encoded[i * 128], params));
        }
    }
    state.SetItemsProcessed(state.iterations() * n);
}
...
static void BM_SQ8Asymmetric_768d(benchmark::State& state) {
    ...
    auto params = vexdb::sq8_compute_params(vecs.data(), n, 768);
    std::vector<uint8_t> encoded(n * 768);
    for (int i = 0; i < n; i++) {
        vexdb::sq8_encode(&vecs[i * 768], &encoded[i * 768], params);
    }
    ...
    for (auto _ : state) {
        for (int i = 0; i < n; i++) {
            benchmark::DoNotOptimize(
                vexdb::sq8_asymmetric_l2(query.data(), &encoded[i * 768], params));
        }
    }
    state.SetItemsProcessed(state.iterations() * n);
}
